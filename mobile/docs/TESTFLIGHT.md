...
| EAS Update | live, branch `production`, runtimeVersion policy `appVersion`; last group `bb49b0fc-68ac-4690-a91c-92b7a6f991eb` (2026-09-12, runtime `1.0.4`, `gitCommitHash 8a5273be2` on branch `ota/1.0.4-combined` = main `b4828c28a` with `app.json` held at 1.0.4) — see the publish log below. `main` targets 1.0.5; until build 45 is live, publish 1.0.4 groups by re-cutting that branch from main with only the version line held back, never from an older fork |
