...
| 2026-09-19 | `3e1d0edf-fc48-489c-9142-545a55a9cc1c` | `d7120f9a6` | Home stops its session-activity clocks while a session screen covers it. Home stays mounted under the push, so every working row kept re-evaluating about a hundred Reanimated styles per frame behind another screen; `SessionActivityPane` now carries visibility down and `useSessionActivity` gates the clock on it. Idle rows also stop registering an `AppState` listener each, and `buildSessionTree` no longer de-duplicates roots with `indexOf` per node (O(n^2) on every status frame). | `git diff f621353bb d7120f9a6 -- mobile/package.json mobile/app.json mobile/app.config.js mobile/plugins mobile/modules mobile/patches` is empty, so no native surface moved since build `f621353bb`. Mobile type check and 31 native check files green; Maestro flow `e2e/session-activity.yaml` green on the iPhone 17 simulator before the rebase. Published by mobile-ota run `35430371248`; group, runtime `1.0.10` and both platform records read back with `eas update:view`. NOT MEASURED: no benchmark of the activity grid exists yet, so the size of the saving is unproven. `bench/session-activity-entry.tsx` is the instrument and has never run. |
| 2026-09-19 | `b127f1cd-71b7-44ec-a809-1280248cebe7` | `19251b5d1` (`main`) | Session menu gains Rename with AI, directly under Rename, which calls `POST /api/sessions/:id/title/generate`. Automatic titles at spawn time and on this trigger now come back non-empty: the request turns reasoning off and asks for 512 tokens, and the hosted proxy no longer relays `content-encoding: zstd` over a plaintext body (vibes #1682). Also carries the three-line composer fix. | Runtime `1.0.11`. `git diff 42adf237c 19251b5d1 -- mobile/package.json mobile/app.json mobile/app.config.js mobile/plugins mobile/modules mobile/patches` is empty, and `42adf237c` is the commit the 1.0.11 native build (mobile-release run `35446195209`) was cut from. Endpoint verified on the restarted local service: 200 with a real title, after the vibes control-plane deploy. Menu row checked on the iPhone 17 Pro simulator in session 4217a92d before landing. Published by mobile-ota run `35455468042`; group, runtime and commit read back from the publish step. |
| 2026-09-19 | `e05fc794-c6d8-4b11-9df8-791add1a68b4` | `ac4662ac8` (`release/ios-1.0.10-title`) | The same Rename with AI row and composer fix for phones still on runtime 1.0.10. | Branch is `f8a151448` (the last 1.0.10 main) plus cherry-picks of `fbdb90ccc` and `87fc5f8b4`. It omits `d630e91f8`, which adds `@shopify/react-native-skia`, a native module the 1.0.10 binary does not contain. `git diff f621353bb ac4662ac8 -- mobile/package.json mobile/app.json mobile/app.config.js mobile/plugins mobile/modules mobile/patches` is empty. Published by mobile-ota run `35455493911`; group, runtime `1.0.10` and commit read back from the publish step. Not checked on a device before publishing. |
| 2026-09-20 | `33abb2b9-00f3-4739-aa63-825f779a6c8e` | `e0bf9c7a0` (`main`) | The omg agent's model picker shows a short name with the provider's mark ("DeepSeek V4 Flash", "GLM 5.2", "GPT-5.6 Sol"; `packages/protocol/src/omg-model-display.ts` owns the rule, router ids stay the value). With a hosted model chosen, the composer avatar is the provider's mark with a small omg badge. The omg usage ring shows the free plan's signup credit (server change, v0.6.80). | Runtime `1.0.11`. `git diff 42adf237c e0bf9c7a0 -- mobile/package.json mobile/app.json mobile/app.config.js mobile/plugins mobile/modules mobile/patches` is empty (PNG provider marks ride the update). Mobile type check and 31 native check files green. Maestro flow `e2e/omg-models.yaml` green on a release build (`--build`) on the iPhone 17 Pro simulator; composer badge checked by screenshot. Published by mobile-ota run `35491299133`; group, runtime `1.0.11`, both platforms and commit read back from the publish step. |
| 2026-09-20 | `51e0f981-a7e0-44a1-aaf9-d1b2a80c0834` | `d674f9fe0` (`main`) | The omg agent offers a thinking level (low, medium, high) on models where the hosted router honours it (DeepSeek, Z.ai, Anthropic, OpenAI); Qwen3.7 Plus, MiniMax M3 and Qwen3 Coder Next show no slider. Levels arrive per model in `/api/coding-agents` (`thinkingLevelsByModel`), so the picker needs the v0.6.82 box. The agent picker sheet keeps one height (`Sheet resizable={false}`): the expanded tray around a model list that kept its own height was hard to scroll. | Runtime `1.0.11`. `git diff 42adf237c d674f9fe0 -- mobile/package.json mobile/app.json mobile/app.config.js mobile/plugins mobile/modules mobile/patches` is empty. Mobile type check and 31 native check files green. Maestro flow `e2e/omg-models.yaml` green on a release build on the iPhone 17 Pro simulator against the deployed v0.6.82 box: slider present for DeepSeek and GLM, absent for Qwen3 Coder Next. Published by mobile-ota run `35510516229`; group, runtime `1.0.11`, both platforms and commit read back from the publish step. |
| 2026-09-20 | `0a0e9b9f-2a69-4c1f-bac7-703b2ed64441` | `ae86abe55` (`main`) | Streaming drafts reuse completed transcript rows. Home uses a bounded list window and pauses offscreen activity. Opening mounts the latest 12 grouped items, seeded from the existing memory cache, then expands local history before fetching more. | Runtime `1.0.11`; native surface and lockfile unchanged from build 54 (`42adf237c`). 72 focused tests, 33 native check files and mobile/root type checks pass; one existing native quarantine. Recorded release simulator plan passed 6/6; supplemental Maestro flow verified local history expansion, server pagination, and a deep Home row. Full-suite failures match the prior baseline. Streaming fixture JS p95: 91.73 to 17.41 ms; no physical-device FPS claim. Workflow `35492403389` succeeded. Group, both platforms, runtime and commit independently read back with `eas update:view`. Phone activation not verified. |
| 2026-09-20 | `1cf9ab2c-840a-47e8-b67b-65ac2fa7b93b` | `8aead35c5` (`main`) | Rename with AI disappears from the session menu when the machine sets `autoSessionTitles` to `off`. The server answers 403 in that mode, so the row could only have failed. The setting is new, defaults to `on`, and is edited on the web (Settings, More, View); the phone only reads it. | Runtime `1.0.11`. `git diff 42adf237c 8aead35c5 -- mobile/package.json mobile/app.json mobile/app.config.js mobile/plugins mobile/modules mobile/patches` is empty, and `42adf237c` is the commit the 1.0.11 native build (mobile-release run `35446195209`) was cut from. Mobile type check clean and 33 native check files green. All three modes verified against the restarted local service: `on` returns 200 with a title, `off` returns 403, an unknown value returns 400. Published by mobile-ota run `35501034683`; group, runtime and commit read back from the publish step. NOT PUBLISHED to runtime 1.0.10: the two OTAs earlier today went to 1.0.11 only, so this follows them rather than reviving the maintenance branch. NOT CHECKED on a device. |
| 2026-09-21 | `29073631-a5d8-47d1-8779-8f34e6241d39` | `5a7fe1265` | Settings gains a native Coding agents screen: the roster with its state, Connect in place for Claude Code and Codex through the box's own `/api/coding-agents/:kind/auth` routes, a Claude account detail page with Disconnect, and one tray that does a step at a time. Onboarding's connect step is the same list. The control-plane OAuth path is gone. | **This group DOES move the native surface, unlike every row above it.** `mobile/package.json` gains `expo-web-browser`, which has an iOS podspec and Swift sources, so no installed binary has `ExpoWebBrowser`. Published anyway, on Benny's explicit decision, because the module is guarded the way `expo-iap` was cleared on 2026-08-18: `in-app-browser.ts` asks `requireOptionalNativeModule("ExpoWebBrowser")` and falls back to `Linking.openURL` (Safari) when the answer is null. That guard is the fix for a real crash the same day -- a lazy `require` inside a `try` does NOT hold, because Metro's `guardedLoadModule` gives the load failure to `ErrorUtils.reportFatalError` instead of throwing it back, and the app aborted with "Cannot find native module 'ExpoWebBrowser'". `scripts/in-app-browser.native-check.ts` covers both answers. So on an old binary the sign-in page opens in Safari; the in-app sheet needs a new build through `mobile-release.yml`. `app.json` and `app.config.js` unchanged. Onboarding (15/15) and coding-agents (4/4) plans green on the iPhone 17 Pro simulator. Published by mobile-ota run 35588041207; runtime `1.0.12`, ios+android, group and commit read from the publish step. |
| 2026-09-21 | `2a1f205b-7240-4622-8ad5-fafd22e42f0c` | `77b37a042` (`ota/app-builder-skill-1.0.12`) | New Project App and Website prompts name `.agents/skills/omg-app-builder/SKILL.md` directly. An existing target folder receives the managed skill before its session starts. | Re-cut from the last compatible app-builder baseline (`0cd805ea7`) because current `main` adds the native `react-native-webview` package. The maintenance branch has no native-surface change. Workflow `35598690349` passed type check, native checks, Metro export, and the native gate. `eas update:view --json` independently confirmed production, runtime `1.0.12`, iOS and Android, group, and commit. Not checked on a device. |
| 2026-09-21 | `c05fd623-ee0a-4ea0-9245-fda63b0bda78` | `ef8b60cb5` (`ota/app-builder-skill-1.0.12`) | Expo New Project now uses a portable delivery ladder: hosted web preview first, Expo Go tunnel QR for device testing when available, a development build for unsupported native modules, and TestFlight only on explicit request. It no longer requires a local simulator, Xcode, or a Mac. | Forward update of the compatible `1.0.12` app-builder branch; no native-surface change. Workflow `35634747425` passed type check, native checks, Metro export, and the native gate. `eas update:view --json` independently confirmed production, runtime `1.0.12`, iOS and Android, group, and commit. Not checked on a device. |
