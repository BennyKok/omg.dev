...
/** `atFullHeight`: the sheet cannot grow, so an upward drag over a scroller belongs to it. */
export function canDragSheet(dx: number, dy: number, origin: SheetTouchOrigin | null, atFullHeight: boolean): boolean {
  ...
  return !origin || dy > 0 || !atFullHeight;
}
