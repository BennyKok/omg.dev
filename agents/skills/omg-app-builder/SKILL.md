...
- Start Metro once with `BROWSER=none EXPO_PACKAGER_PROXY_URL=<expoGo.proxyUrl> npx expo start --go --web --host lan --port <port>`. Use the project's local Expo dependency. Keep this process running.
- Verify the exact port locally and verify that the HTTPS proxy URL answers. Give the user `expoGo.url`, which starts with `exps://`, for Expo Go. The owner preview card uses the same live Metro server for Expo Web.
...
- The Expo Go capability link is short-lived. If it expires, stop Metro, call `omg_expose_port` again with `expoGo: true`, and restart Metro with the new proxy URL.
