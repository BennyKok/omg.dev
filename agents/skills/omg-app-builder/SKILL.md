...
- For an Expo app, start from the managed `expo` template. Adapt its screen and API route instead of rebuilding configuration. Keep API secrets in server routes. A production native build needs the server deployed at a secure origin; do not treat a sandbox tunnel as production hosting.
...
- When device testing is useful and the app is compatible with Expo Go, start `npx expo start --tunnel` and give the user its Expo Go QR code. The tunnel is the native device path and must stay running while the user tests. Do not assume the user shares the sandbox network. If the tunnel needs an Expo login or another user action, use the supported login flow. If Expo Go cannot load a required native module, use a development build when authorized or report that limit clearly.
...
- After Expo Web is verified, start `npx expo start --tunnel` when the user wants to test on a device. Show the terminal QR code and state that the process must stay active. This tunnel is temporary and is not a deployment. Do not start a second Metro process in the same project until the web preview process is stopped; one Expo dev server can change its connection mode when practical.
